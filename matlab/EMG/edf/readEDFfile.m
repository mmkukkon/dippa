function [recstruct,nsig,duration,nrecs] = readEDFfile(filename,nrecs_arg)
...
fileinfo.duration = str2double(char(fhbuf(end-12:end-4))); %Duration of each data record
...
duration = fileinfo.duration; %Duration of each data record
nrecs = fileinfo.nrecs; %number of data records

if isempty(nrecs_arg)
    RecsToRead = nrecs; %originally: 10
else
    RecsToRead = nrecs_arg;
end
RecsAtATime = 5;
...
% sampspersig = sampspersig * 10; %oma lisäys koodin nopeuttamiseksi (Hanna)
